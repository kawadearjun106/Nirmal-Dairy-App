...
class FarmerMonthlyReportActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_farmer_monthly_report)
        ...
        db.collection("milk_prices").document("current_rate").get()
            ...
            .addOnFailureListener { loadMonthlyEntries() }
    }

    private fun loadMonthlyEntries() {
        ...
        db.collection("milk_entries")
            ...
            .addOnSuccessListener { snapshot ->
                if (snapshot.isEmpty) {
                    tvSummary.text = "No milk entries found for this month."
                ...
                for (doc in snapshot.documents) {
                    ...
                    val farmerName = doc.getString("farmerName") ?: "Farmer"
                ...
                if (count > 0) {
                    ...
                    tvSummary.text = """
                        🧾 Monthly Summary (${SimpleDateFormat("MMMM yyyy", Locale.getDefault()).format(Date())})
                        🥛 Total Milk: ${String.format("%.2f", totalMilk)} L
                        🧈 Avg Fat: ${String.format("%.2f", avgFat)}%
                        💰 Total Earnings: ₹${String.format("%.2f", totalAmount)}
                    """.trimIndent()
                } else {
                    tvSummary.text = "No entries found for this month."
                }
            }
